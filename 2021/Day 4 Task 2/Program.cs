...
namespace Day_4_Task_2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var winningCardsBeforeDraw = cards.Where(c => c.Any(x => x.Count == 0)).ToList();
                cards.ForEach(c => c.ForEach(x => x.Remove(number)));
                var winningCards = cards.Where(c => c.Any(x => x.Count == 0)).ToList();
                if(winningCardsBeforeDraw.Count == cards.Count - 1 && winningCards.Count == cards.Count)
                {
                    var lastCardToWin = cards.SingleOrDefault(x => !winningCardsBeforeDraw.Contains(x));
                    Console.WriteLine($"Winning card is at index: {cards.IndexOf(lastCardToWin)}");
                    var unmarkedSum = lastCardToWin.Sum(x => x.Sum()) / 2; // All numbers appear twice. Once in the row and once in the column list.
